...
// GET ALL (optional filter by ?grade=prep)
export async function GET(req: NextRequest) {
  try {
    ...
    const students = grade
      ? await AdmissionStudentModel.find({ grade })
      : await AdmissionStudentModel.find({});

    // return array directly (your list component accepts both)
...
//   try {
//     const { id } = await req.json(); // expects { id }
//     if (!id) {
//       return NextResponse.json({ error: "Student ID is required" }, { status: 400 });
//     }
